...
const eachCountry = countries.forEach((x) => console.log(x));

// Use forEach to console.log each name in the names array.
const eachNames = names.forEach((x) => console.log(x));
// Use forEach to console.log each number in the numbers array.
const eachNumber = numbers.forEach((x) => console.log(x));
// Use map to create a new array by changing each country to uppercase in the countries array.
const countryMap = countries.map((x) => x.toUpperCase());
console.log(countryMap);
// Use map to create an array of countries length from countries array.
const countryLength = countries.map(
  (x) => `the length of ${x} is ${x.length} `
);
...
const upperName = names.map((x) => x.toUpperCase());
console.log(upperName);
// Use map to map the products array to its corresponding prices.
const mapPrice = products.map((x) => `${x.product} => ${x.price} Dollars`);
console.log(mapPrice);
// Use filter to filter out countries containing land.
const filteredCountry = countries.filter((x) => x.includes("Land"));
console.log(filteredCountry);
// Use filter to filter out countries having six character.
const sixLetCountty = countries.filter((x) => x.length === 6);
console.log(sixLetCountty);
// Use filter to filter out countries containing six letters and more in the country array.
const sixLetCounttyMore = countries.filter((x) => x.length >= 6);
console.log(sixLetCounttyMore);
// Use filter to filter out country start with 'E';
const beginsWithE = countries.filter((x) => x[0] === "E");
console.log(beginsWithE);
// Use filter to filter out only prices with values.
const priceWithValue = products.filter((x) => x.price > 0);
console.log(priceWithValue);
// Declare a function called getStringLists which takes an array as a parameter and then returns an array only with string items.
const getStringLists = numbers.map((x) => x.toString());
console.log(getStringLists);
// Use reduce to sum all the numbers in the numbers array.
const sumNum = numbers.reduce((total, a) => total + a, 0);
console.log(sumNum);
// Use reduce to concatenate all the countries and to produce this sentence: Estonia, Finland, Sweden, Denmark, Norway, and IceLand are north European countries

// console.log(
//   "I cant solve this concat question: Use reduce to concatenate all the countries and to produce this sentence: Estonia, Finland, Sweden, Denmark, Norway, and IceLand are north European countries"
// );
// const stringArray = countries.reduce((a, b) => a.concat(b.), " ");
...
const onlyReduce = products.reduce((a, b) => (a += b.price), 0);
...
const countryPattern = countries1.filter((x) => x.name.includes("land"));
