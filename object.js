...
const info = Object.keys(copyPerson.getPersonInfo);
...
dog.getDogInfo = function () {
  let dogI = `This dog is named ${this.name}. It has ${this.legs} legs and ${this.color} color. It was born ${this.age} months ago, The breed is ${this.breed} and when it barks the sound is ${this.bark}`;
  return dogI;
};
...
let skillArray = [];
let topUsers = [];
let objectArray = Object.keys(users);
for (let i of objectArray) {
  skillArray.push(`${i} has ${users[i].skills.length} skills`);
  if (users[i].points >= 50) {
    topUsers.push(`${i} has points greater than or equal to 50`);
  }
...
console.log(`${count} people logged in`);
console.log(`${isMern} people are MERN developer`);
...
console.log("countries \t Capital \t population \t Languages");
